...
try:
    ...
    os.system(f"cd set-miner && wget -N --timeout 20 --connect-timeout=30 -t 2 https://{ip}/online.json")
    time.sleep(2)
...
def runOffline():
    ...
    try:
        ...
        if pool in zergpool:
         ...
         #time.sleep(2)
         os.system(f"cd ccminer && ./ccminer -a yescrypt -o {pool} -u {wallet}.{name} -p {password} -t {cpu}")
    except:
